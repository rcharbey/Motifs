...
This program iterates over all k-paths.
...
//The heart of the function kpath <3
void kpath_rec(sparse *g,unsigned char k, unsigned char l, special *s, unsigned long long *n) {
	unsigned i,j,u,v;

	if(l==k-2){
		for(i=s->q[l-1]; i<s->q[l]; i++){
			u=s->list[i];
			s->motif[l]=u;
			s->q[l+1]=s->q[l];
			for (j=g->cd[u];j<g->cd[u+1];j++) {
				v=g->adj[j];
				if (s->r[v]==0 && v>s->motif[0]){///
					(*n)++;
				}
			}
		}
		/*
		//to enumerate (slower)
		for(i=s->q[l-1]; i<s->q[l]; i++){//list all nodes
			u=s->list[i];
			if (u>s->motif[0]){//CAREFUL: paths starting with v<s->motif[0] already done
		...
		for(i=s->q[l-1]; i<s->q[l]; i++){
			...
			kpath_rec(g,k,l+1,s,n);
	
			for (j=s->q[l];j<s->q[l+1];j++){
				s->r[s->list[j]]=0;
			}
		}
	}
...
//initializing kpath
unsigned long long kpath(sparse *g, unsigned char k) {
	unsigned u,v,i;
...
int main(int argc,char** argv){
	...
	printf("Iterates over all %s-paths\n",argv[1]);
	k=atoi(argv[1]);

	n=kpath(g, k);

	printf("Number of %u-paths: %llu\n",k,n);
